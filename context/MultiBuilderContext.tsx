...
const MultiBuilderContext = createContext<MultiBuilderContextType | undefined>(undefined);

export const useMultiBuilder = () => {
  const context = useContext(MultiBuilderContext);
  if (!context) {
    throw new Error('useMultiBuilder must be used within a MultiBuilderProvider');
  }
  return context;
};
...
export const MultiBuilderProvider: React.FC<MultiBuilderProviderProps> = ({ children }) => {
  ...
  const initializeData = async () => {
    ...
    try {
      ...
      Alert.alert('Error', `Failed to initialize ${selectedSport.toUpperCase()} data`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchMatchups = async (sport: string) => {
    ...
    try {
      ...
      const matchupsData = sport === 'nrl' ? nrlUpcomingMatchups : aflUpcomingMatchups;
      ...
      if (matchupsData && Array.isArray(matchupsData)) {
        ...
        console.log(`Loaded ${matchupArray.length} matchups for ${sport}:`, matchupArray);
        setMatchups(matchupArray);
      } else {
        console.error(`${sport.toUpperCase()} matchups data is not valid:`, matchupsData);
  ...
  const fetchPlayers = async (sport: string, matchup: string) => {
    ...
    try {
      ...
      console.log(`Fetching players for teams: ${teams.join(', ')} in ${sport}`);
  ...
  const getDynamicCoScorers = async (
    ...
    try {
      ...
      // Get the appropriate data based on sport
      const coScoringSummaries = selectedSport === 'nrl' ? nrlCoScoringSummaries : aflCoScoringSummaries;
      const matchupsData = selectedSport === 'nrl' ? nrlUpcomingMatchups : aflUpcomingMatchups;
      
      console.log(`Getting co-scorers for ${player} (${team}) in sport: ${selectedSport}`);
      ...
      if (coScoringSummaries && coScoringSummaries.length > 0) {
        ...
        const playerCoScorers = coScoringSummaries
          .filter((data: CoScorerData) => {
            const nameMatch = data.primary_player.toLowerCase().includes(player.toLowerCase()) || 
                             player.toLowerCase().includes(data.primary_player.toLowerCase());
            return nameMatch;
          })
          .sort((a: CoScorerData, b: CoScorerData) => 
        ...
        if (playerCoScorers.length > 0) {
          ...
          const filteredCoScorers = upcomingPlayers.length > 0
            ? topCoScorers.filter((data: CoScorerData) => 
                upcomingPlayers.some(p => 
      ...
      if (recommendations.length < 5) {
        try {
          ...
          if (queryEmbedding.length > 0) {
            ...
            const playerEmbeddings: {player: string; embedding: number[]; similarity: number}[] = [];
      ...
      if (recommendations.length < 5) {
        ...
        const teamPlayers = matchupsData
          .filter((match: MatchupData) => {
            const teamName = selectedSport === 'nrl' ? match["Team Name"] : match.Team;
            return teamName === team;
          })
          .map((match: MatchupData) => {
            return selectedSport === 'nrl' ? match["Player Name"] : match.Player;
          })
        ...
        console.log(`Using fallback: Found ${uniqueTeamPlayers.length} potential co-scorers from team`);
  ...
  const getLossScorers = (
    ...
    try {
      ...
      // Get the appropriate data based on sport
      const matchupsData = selectedSport === 'nrl' ? nrlUpcomingMatchups : aflUpcomingMatchups;
      const defeatsScorerData = selectedSport === 'nrl' ? nrlTryscorersInDefeats : aflGoalscorersInDefeats;
      ...
      console.log(`Found ${oppositeTeamPlayers.size} players in opposite team: ${oppositeTeam}`);
      ...
      if (defeatsScorerData) {
        ...
        const teamLossScorers = defeatsScorerData
          .filter((data: any) => {
            ...
            const inOppositeTeam = playerName && oppositeTeamPlayers.has(playerName);
            const hasLossGames = selectedSport === 'nrl' ? 
          ...
          .sort((a: any, b: any) => {
            ...
            const percentageKey = selectedSport === 'nrl' ? 'loss_try_percentage' : 'Percentage';
            const percentageA = parseFloat(String(a[percentageKey]).replace('%', ''));
            const percentageB = parseFloat(String(b[percentageKey]).replace('%', ''));
            return percentageB - percentageA;
          });
        
        console.log(`Found ${teamLossScorers.length} loss scorers for ${oppositeTeam}`);
        ...
        if (teamLossScorers.length > 0) {
          ...
          // Filter by upcoming players if available
          const filteredLossScorers = upcomingPlayers.length > 0
          ...
          for (const scorer of filteredLossScorers) {
            const playerKey = selectedSport === 'nrl' ? 'full_name' : 'Player';
            const percentageKey = selectedSport === 'nrl' ? 'loss_try_percentage' : 'Percentage';
            const totalGamesKey = selectedSport === 'nrl' ? 'total_loss_games' : 'Total_Defeats';
      ...
      if (recommendations.length < 3) {
        ...
        const remainingPlayers = Array.from(oppositeTeamPlayers as Set<string>)
          .filter((player: string) => !recommendations.some(rec => rec.name === player));
  ...
  const getHomeScorers = (
    ...
    try {
      ...
      // Get the appropriate data based on sport
      const matchupsData = selectedSport === 'nrl' ? nrlUpcomingMatchups : aflUpcomingMatchups;
      const homeAwayData = selectedSport === 'nrl' ? nrlHomeAwayTrycorers : aflHomeAwayGoalscorers;
      
      console.log(`Getting home scorers for ${homeTeam} in sport: ${selectedSport}`);
      ...
      if (homeAwayData) {
        ...
        console.log(`Found ${homeTeamPlayers.size} players in home team: ${homeTeam}`);
        
        // Step 2: Filter home/away data for players in the HOME team
        const playerKey = selectedSport === 'nrl' ? 'full_name' : 'Player';
        let filteredHome = homeTeamPlayers.size > 0
          ? homeAwayData.filter((data: any) => 
              homeTeamPlayers.has(data[playerKey]))
          : homeAwayData;
        
        console.log(`Found ${filteredHome.length} home scorers in data for ${homeTeam}`);
        
        // Step 3: Exclude the selected player
        if (player) {
          filteredHome = filteredHome.filter((data: any) => 
            data[playerKey] !== player);
        }
        
        // Step 4: Sort by home percentage in descending order
        const percentageKey = selectedSport === 'nrl' ? 'home_try_percentage' : 'Home_Percentage';
        ...
        for (const scorer of topHomeScorers) {
          const homeGamesKey = selectedSport === 'nrl' ? 'home_games' : 'Home_Games';
          let percentage = scorer[percentageKey];
        ...
        if (upcomingPlayers && upcomingPlayers.length > 0) {
          ...
          if (filteredRecommendations.length > 0) {
            ...
            const commonPlayers = Array.from(homeTeamPlayers as Set<string>).filter((player: string) => 
              upcomingPlayers.includes(player)
            );
              ...
              for (let i = 0; i < Math.min(3, commonPlayers.length); i++) {
                ...
                if (playerData) {
                  ...
                  recommendations.push({
                    ...
                    percentage: percentage,
                    home_games: parseFloat(String(playerData[homeGamesKey] || 0))
                  });
                } else {
                  // Generate realistic stats if not available
                  recommendations.push({
                    name: playerName as string,
                    percentage: Math.round((50 - i * 5 + (Math.random() * 10 - 5)) * 10) / 10,
  ...
  const ensureUniqueRecommendations = (result: AnalysisResult): AnalysisResult => {
    // Get all players from co_scorers
    const coScorerPlayers = new Set(result.co_scorers.recommendations.map(rec => rec.name));
    ...
    if (result.home_scorers && result.home_scorers.recommendations) {
      ...
      result.home_scorers.recommendations = result.home_scorers.recommendations.filter(
        rec => !coScorerPlayers.has(rec.name)
      );
      const removedCount = originalCount - result.home_scorers.recommendations.length;
      if (removedCount > 0) {
        console.log(`Removed ${removedCount} duplicate players from home_scorers`);
      }
    }
    ...
    if (result.loss_scorers && result.loss_scorers.recommendations) {
      ...
      result.loss_scorers.recommendations = result.loss_scorers.recommendations.filter(
        rec => !allPlayers.has(rec.name)
      );
      const removedCount = originalCount - result.loss_scorers.recommendations.length;
      if (removedCount > 0) {
        console.log(`Removed ${removedCount} duplicate players from loss_scorers`);
      }
    }
  ...
  const analyzeMultiBuilder = async (
    ...
    try {
      ...
      console.log(`Analyzing ${sport} multi: ${matchup}, lock: ${player}, winner: ${winner}`);
      ...
      const coScorers = await getDynamicCoScorers(player, winner, allMatchupPlayers);
      const lossScorers = getLossScorers(oppositeTeam, allMatchupPlayers);
      // Pass matchup to getHomeScorers to extract home team
      const homeScorers = getHomeScorers(matchup, winner, allMatchupPlayers, player);
      ...
      const scoringTerm = sport.toLowerCase() === 'nrl' ? 'Try' : 
                         sport.toLowerCase() === 'afl' ? 'Goal' : 'Touchdown';
      const scoringTermLower = scoringTerm.toLowerCase();
      const scoringTermPlural = sport.toLowerCase() === 'nrl' ? 'tries' : 
                               sport.toLowerCase() === 'afl' ? 'goals' : 'touchdowns';
  ...
  useEffect(() => {
    ...
    if (process.env.EXPO_PUBLIC_OPENAI_API_KEY) {
      ...
      console.log(`Key preview: ${key.substring(0, 3)}...${key.substring(key.length-3)}`);
    }
  }, []);
